 ...
 */
public class DeleteTestAppComp extends ComponentDefinition {
    ...
    protected final Handler<AppCompTimeout> handleTimeout = new Handler<AppCompTimeout>() {
        ...
        public void handle(AppCompTimeout event) {
            ...
            if((counter < numberOfInsertions && rand == 1) || (counter < numberOfInsertions && deletionCounter >= numberOfDeletions )){
                clock++;
            ...
            if((deletionCounter < numberOfDeletions && rand == 2 && document.getDocumentLines().size() > 0)
                    || (deletionCounter < numberOfDeletions && counter >= numberOfInsertions && document.getDocumentLines().size() > 0)){
                clock++;
                int deleteIndex = random.nextInt(document.getDocumentLines().size());
                LineId deleteId = document.getIdTable().get(deleteIndex+1);
                CRBBroadcast broadcast = new CRBBroadcast(new Patch(Lists.newArrayList(new Delete(deleteId, document.getDocumentLines().get(deleteIndex)))));
                trigger(broadcast, broadcastPort);
                deletionCounter++;
            }
        }
    ...
    public static class Init extends se.sics.kompics.Init<DeleteTestAppComp> {
        ...
        public Init(KAddress selfAdr, Identifier gradientOId, int numberOfInsertions, int numberOfDeletions) {
