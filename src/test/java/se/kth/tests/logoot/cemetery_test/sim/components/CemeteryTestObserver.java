 ...
 */
public class CemeteryTestObserver extends ComponentDefinition {

    private static final Logger LOG = LoggerFactory.getLogger(CemeteryTestObserver.class);
    ...
    protected final Handler<ObserverTimeout> handleTimeout = new Handler<ObserverTimeout>() {
        ...
        public void handle(ObserverTimeout event) {
            ...
            boolean done = true;
            for (int i = 0; i < numberOfNodes; i++) {
                if (nodesState[i] < numberOfPatchs)
                    done = false;
            }
            if (done) {
    ...
    public static class Init extends se.sics.kompics.Init<CemeteryTestObserver> {
